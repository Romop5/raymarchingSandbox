...
auto
InterpolatedCamera::GetTransformation() const -> const glm::mat4&
{
  return currentTransformation;
}

auto
InterpolatedCamera::UpdateFrame(size_t frameID) -> void
{
  /* Camera position is calculated as linear interpolation of two successive
   * camera points Each point has 'frame' member, which defines start of the
   * camera point in sequence. */
  ...
  auto t =
    GetFrameIDAsInterpolationParameter(frameID, startP.frame, endP.frame);
  ...
  auto rotation = glm::mat4(glm::mat3(
    glm::normalize(glm::cross(upVector, lookVector)), upVector, lookVector));

  currentTransformation = glm::translate(-position) * rotation;
}

auto
InterpolatedCamera::GetIndexesForFrameID(size_t frameID)
...
auto
InterpolatedCamera::GetFrameIDAsInterpolationParameter(size_t frameID,
                                                       size_t start,
                                                       size_t end) -> float
{
  if (frameID > end) {
