 ...
 *
 * This camera is useful for pre-created sequences, which should be
 * deterministic, e.g. due to repeatability or for accurate performance
 * tracking.
 *
...
private:
  ...
  /// Calculates interpolation parameter 't' from current frameID and current
  /// start/end frame times
  auto GetFrameIDAsInterpolationParameter(size_t frameID,
                                          size_t start,
                                          size_t end) -> float;
