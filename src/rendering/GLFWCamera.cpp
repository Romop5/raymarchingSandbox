...
raymarcher::GLFWCamera::GLFWCamera(std::shared_ptr<ICamera> cameraPtr,
...
auto
GLFWCamera::GetType() const -> CameraType
{
  return type;
}

auto
GLFWCamera::UpdateFrame() -> void
{
...
auto
GLFWCamera::GetTransformation() const -> const glm::mat4&
{
  return camera->GetTransformation();
}

auto
GLFWCamera::MouseCursorChanged(GLFWwindow* window,
                               double relativeX,
                               double relativeY) const -> void
{
...
auto
GLFWCamera::ScrollChanged(GLFWwindow* window,
                          double relativeX,
                          double relativeY) const -> void
{
  if (type == ORBITER_CAMERA) {
...
auto
GLFWCamera::KeyPressed(GLFWwindow* window, int key) const -> void
{
  if (type == FLYING_CAMERA) {
    auto& flyingCamera = static_cast<FlyingCamera&>(*camera);
    ...
    switch (key) {
      ...
      case GLFW_KEY_KP_ADD:
        flyingCamera.SetAngularSpeed(flyingCamera.GetAngularSpeed() *
                                     angleSpeedIncrement);
        break;
      case GLFW_KEY_KP_SUBTRACT:
        flyingCamera.SetAngularSpeed(flyingCamera.GetAngularSpeed() *
                                     (1.0f / angleSpeedIncrement));
        break;
      default:
        break;
    }
