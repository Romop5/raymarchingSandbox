...
namespace raymarcher 
{
     ...
     *
     * This class is used for both primitives (spheres) and bouding volumes (spheres)
     * IsBoudingSphere() method is used to differentiate between each other
     */
    class SpherePrimitive
    {
        ...
        /// Steal subnode's children if current vs node's volume ratio is bigger than threshold
    ...
    /**
     * @brief Implements manipulation, optimization and code generation of sphere-based scene
     */
    class BVHLibrary
    {
        public:
        ...
        {
            /// Remove one level of tree when ratio of parent-child volume is bigger than 
         ...
         * @param optimized determines if resulting code should take BVH into account, or print out
         * just the leaves (without any acceleration)
         */
         ...
         *
         * This function takes a flat array of nodes in the scene, and constructs a full binary BVH
         * tree. Subsequently, the tree is optimized using parameters 'params'.
         *
         * Two levels of optimization are employed (in the following order):
         * - pseudo Surface Area Heuristic
         *   By comparing volumes of parent and child node, it can be detected what's the probability
         *   that the SDF will be evaluated for the child provided it was evaluated for parent.
         *   
         *   The idea is that if child has similar radius to parent, then it's useless in BVH
         *   hierarchy, and it can be removed by moving its children to parent.
         *
         * - inflating since fixed level: e.g. if maxLevel is 5 and the BVH tree has 12 levels, all
         *   subtrees are flattened since level 5
         */
        auto Optimize() -> void;
        protected:
