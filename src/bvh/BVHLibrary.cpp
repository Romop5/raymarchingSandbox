...
namespace {
auto
GetSphereVolume(float r) -> float
{
  return 1.3333333 * glm::pi<float>() * pow(r, 3.0f);
}

auto
GetSphereToSphereVolumeRatio(float smallerRadius, float biggerRadius) -> float
{
...
auto
SpherePrimitive::GetChildren() -> ChildrenType&
{
  return children;
}

auto
SpherePrimitive::GetCenter() -> glm::vec3
{
  return center;
}

auto
SpherePrimitive::GetSize() -> float
{
  return size;
}

auto
SpherePrimitive::IsBoudingSphere() const -> bool
{
  return isBoundingSphere;
}

double
SpherePrimitive::DistanceTo(SpherePrimitive& other)
{
  // Note: we add radii to prevent a single, large sphere to be close to all
  // spheres due to it's large radius This "distance function" prefers small and
  // close spheres to large-to-small sphere distance
  auto d = glm::length(other.GetCenter() - GetCenter()) +
           (other.GetSize() + GetSize());
  return d;
}

auto
SpherePrimitive::InflateChildren() -> void
{
...
auto
SpherePrimitive::InflateSinceLevel(size_t level) -> void
{
  if (level == 0) {
    InflateChildren();
    return;
...
auto
SpherePrimitive::InflateUsingSAH(float threshold) -> void
{
  ...
  // Until convergence: while there is any subnode's child that has been added
  // to its parent
  do {
    bool hasAddedChild = false;
    ...
    for (auto& child : children) {
      ...
      if (child->IsBoudingSphere() &&
          GetSphereToSphereVolumeRatio(childSize, size) > threshold) {
...
auto
SpherePrimitive::ConstructBoudingBoxForChildren() -> void
{
  center = glm::vec3(0.0);
  for (auto child : children) {
  ...
  for (auto child : children) {
    size = std::max(
      size, glm::length(center - child->GetCenter()) + child->GetSize());
  }
}
...
//------------------------------------------------------------------------
auto
BVHLibrary::GenerateCodeForNode(SpherePrimitive& node, size_t level)
  -> std::string
{
  ...
  if (node.GetChildren().size() == 0) {
    ...
    nodeString << "sphere(pos-" << position << ", " << node.GetSize() << ")";
    ss << padding << "if ( " << nodeString.str() << " < " << minDistance << ")"
       << std::endl;
    ss << padding << "{" << std::endl;
    ...
    ss << padding << "}" << std::endl;
    ss << padding << "else { d = smin(d, " << nodeString.str() << ", k); }"
       << std::endl;
  }
  return ss.str();
}
...
auto
BVHLibrary::GenerateCodeForScene() -> std::string
{
  std::stringstream ss;
...
auto
BVHLibrary::GenerateCodeForNodeNonOptimized(SpherePrimitive& node, size_t level)
  -> std::string
{
  std::string padding(level * 2, ' ');
  std::stringstream ss;
...
auto
BVHLibrary::GenerateCodeForSceneNonOptimized() -> std::string
{
  std::stringstream ss;
...
auto
BVHLibrary::GenerateCode(bool optimized) -> std::string
{
  if (optimized)
...
auto
BVHLibrary::GetScene() -> SpherePrimitive::ChildrenType&
{
  return scene;
}

auto
BVHLibrary::SetScene(SpherePrimitive::ChildrenType scene) -> void
{
  this->scene = std::move(scene);
}

auto
BVHLibrary::SetParams(OptimizationParameters& params) -> void
{
  this->params = params;
}

auto
BVHLibrary::GetParams() const -> const OptimizationParameters&
{
  return this->params;
}

auto
BVHLibrary::Optimize() -> void
{
...
BVHLibrary::ConstructTree() -> void
{
  while (scene.size() > 1) {
    ...
    /*
     * Remove found nodes from current level and insert a new, joined node with
     * two subnodes
     */
    ...
    scene.emplace_back(std::make_shared<SpherePrimitive>(
      SpherePrimitive::ChildrenType{ firstElement, secondElement }));
  }
}
