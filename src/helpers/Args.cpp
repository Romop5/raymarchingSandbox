...
void
Arguments::First(argType type)
{
  this->argumentPrecedence = type;
}

void
Arguments::AddFixed(std::string name, std::string format)
{
  this->fixedArguments.push_back(std::make_pair(name, format));
}

void
Arguments::AddOpt(std::string name,
                  std::string shortKey,
                  bool hasParam,
                  std::string format)
{
...
void
Arguments::AddLongOpt(std::string name,
                      std::string key,
                      std::string shortKey,
                      bool hasParam,
                      std::string format)
...
Arguments::Parse(int argc, const char** argv)
{
      ...
      case STEP_FIXED:
        for (auto& fixedArg : this->fixedArguments) {
          ...
          if (*argumentPointer == NULL || *argumentPointer[0] == '-') {
            errorMessage =
              std::string("Expected fixed argument '") + fixedArg.first + "'";
            return ARGUMENT_INVALID_FORMAT;
          }
          if (!HasFormat(fixedArg.second, *argumentPointer)) {
            errorMessage =
              std::string("Format mismatch for '") + fixedArg.first + "'";
            return ARGUMENT_INVALID_FORMAT;
          }
          ...
          if (*argumentPointer[0] == '-') {
            ...
            if (HasOption(omitMinus)) {
              ...
              if (HasOptionParam(omitMinus)) {
                ...
                if (param == NULL) {
                  errorMessage =
                    std::string("Expected param for option -") + omitMinus;
                  return ARGUMENT_INVALID_FORMAT;
                }
              ...
              errorMessage =
                std::string("Non-defined option '") + omitMinus + "'";
              return ARGUMENT_INVALID_FORMAT;
            }
            argumentPointer++;
            continue;
  ...
  if (*argumentPointer != NULL) {
    errorMessage =
      std::string("Expected no argument, got '") + *argumentPointer;
    return ARGUMENT_INVALID_FORMAT;
  }
  return OK;
}
...
bool
Arguments::HasFormat(std::string format, std::string testedWord)
{
...
bool
Arguments::HasOption(std::string option)
{
  return (this->options.find(option) != this->options.end());
}

bool
Arguments::HasOptionParam(std::string longOpt)
{
...
std::string
Arguments::GetOptionName(std::string opt)
{
...
std::string
Arguments::operator[](std::string argument)
{
  return this->arguments[argument];
}

bool
Arguments::HasArgument(std::string name)
{
...
bool
Arguments::AllowRedefinition(bool allow)
{
...
std::string
Arguments::Value_or(std::string argument, std::string defaultValue)
{
